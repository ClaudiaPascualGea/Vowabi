...
else
{
  ...
  try{
    ...
    $mysql = "select * from usuario where Email='" . $usu . "' AND Admin=1";
      ...
      if( mysqli_num_rows($res)>0 && $row['Password'] == $pwd )
      {
          ...
          if( $res = mysqli_query( $link, $mysql ) )
          {
            $R = array('resultado' => 'ok', 'clave' => $key,
                       'email' => $usu, 'nombre' => $row['Nombre'], 'id' => $row['id']);
          }
          else
          {
            $RESPONSE_CODE = 500;
        ...
        {
            $RESPONSE_CODE = 401;
            $R = array('resultado' => 'error', 'descripcion' => 'Alguno de los campos es incorrecto.');
        }
...
try {
    ...
    $rtn = array('resultado' => 'error', 'descripcion' => "Se ha producido un error");
