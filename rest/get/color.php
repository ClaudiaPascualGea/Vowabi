...
//   rest/color/{ID_color} -> devuelve toda la información del color
//   rest/color/?all={BOOL} -> devuelve todos los colores
...
// =================================================================================
if(is_numeric($ID))
{ // Se devuelve el color por ID

  $mysql = 'select c.* FROM color c
            WHERE c.id = '.$ID.';';
}
...
{ // Se utilizan parámetros
  if( isset($PARAMS['all']) )
  { // se piden todos los colores
      $mysql = 'select c.* FROM color c ORDER BY c.Orden;';
  }
  else
  {
